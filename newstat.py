...
class Parser(object):
    ...
    def __init__(self, stats, category, ip):
        ...
        self.access_reg = re.compile('\[([^ ]+) .*\] /([^ ]+) ([^ ]+) ([^ ]+) \[')
        self.error_reg = re.compile(
            '(.+) \[error\].*(GET|POST) /([^ "\?]+).* HTTP/1\.1.*subrequest: "/([^ "\?]+).*upstream')
    ...
    def __access(self, line):
        ...
        if result != None:
            ...
            qps_key = 'access.qps.' + interface
            self.stats.incr(qps_key, timestamp)

            rtime = float(result.group(4))
            time_key = 'access.response_time.' + interface
    ...
    def __error(self, line):
        ...
        if result != None:
            ...
            key = 'error.' + interface + '_subreqs_qps.' + subrequest
    ...
    def __subreq(self, line):
        ...
        if result != None:
            ...
            qps_key = 'subreq.qps.' + interface
            self.stats.incr(qps_key, timestamp)
            time_key = 'subreq.response_time.' + interface
            self.stats.timing(time_key, timestamp, rtime * 1000)
            if status != 200:
                exception_status_key = 'subreq.exception_status_qps.' + interface
                self.stats.incr(exception_status_key, timestamp)

            if self.ip != 'localhost':
                ip = self.ip.replace('.', '_')
                percentile_key = 'subreq.percentile.' + interface + '.' + ip
